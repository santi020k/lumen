...
const propItems: DocsSearchItem[] = componentDocs.flatMap(component => component.apiReference.map(row => ({
  ...
  keywords: normalizeKeywords(
    component.name, row.attribute, row.values, row.defaultValue, row.description
  ),
  title: `${component.name}: ${row.attribute}`,
  type: 'Prop' as const
})))

const keyboardItemsFor = (
  component: ComponentDoc
): DocsSearchItem[] => (component.keyboardInteractions ?? []).map(row => ({
  category: component.name,
  description: row.action,
  href: `/docs/components/${toSlug(component.name)}#keyboard-title`,
  keywords: normalizeKeywords(
    component.name, row.key, row.action, 'keyboard interaction shortcut'
  ),
...
const eventItems: DocsSearchItem[] = runtimeEvents.map(event => {
  ...
  return {
    ...
    href: owner ?
      `/docs/components/${toSlug(owner.name)}#runtime-events-title` :
      '/docs',
    keywords: normalizeKeywords(
      event.name, event.target, event.detail, event.when
    ),
    title: event.name,
    type: 'Event' as const
  }
...
const recipeItems: DocsSearchItem[] = [
  ...
  },
  {
    ...
    keywords: normalizeKeywords(
      'react hook form RHF register controller useController adapter validation errors'
    ),
  ...
  },
  {
    category: 'Forms',
    description:
      'Submit Lumen forms to Astro Actions and render normalized server validation errors.',
    href: '/docs/forms/astro-actions',
    keywords: normalizeKeywords(
      'Astro Actions form POST server validation ActionInputError getActionResult multipart'
    ),
  ...
  },
  {
    category: 'Forms',
    description:
      'Use form-associated Lumen custom elements with FormData, reset, validity, and native fallbacks.',
  ...
  },
  {
    ...
    keywords: normalizeKeywords(
      'theme playground hue accent dark light createThemeFromHue tokens css export preview'
    ),
  ...
  },
  {
    ...
    href: '/docs/icons',
    keywords: normalizeKeywords(
      'icons lucide icon component name alias search catalog svg glyph symbol foundations'
    ),
    title: 'Icons catalog',
    type: 'Recipe'
  },
  {
    category: 'Foundations',
    description:
      'Search the optional Font Awesome Free brand catalog and copy namespaced Lumen icon names.',
    href: '/docs/brand-icons',
    keywords: normalizeKeywords(
      'brand icons logos font awesome company product marks namespaced catalog'
    ),
  ...
  },
  {
    ...
    href: '/changelog',
    keywords: normalizeKeywords(
      'changelog release notes releases versions updates new fixes migration history'
    ),
  ...
  ...frameworkSetups.map(setup => ({
    ...
    keywords: normalizeKeywords(
      setup.label, setup.packageName, setup.note, setup.usage
    ),
  ...
  ...frameworkGuides.map(guide => ({
    ...
    keywords: normalizeKeywords(
      guide.title, guide.packageName, guide.body.join(' '), guide.code
    ),
  ...
  ...elementsApiReference.map(api => ({
    ...
    keywords: normalizeKeywords(
      api.name, api.description, 'elements web components api'
    ),
    title: api.name,
    type: 'Recipe' as const
  })),
