...
const componentItems: DocsSearchItem[] = componentDocs.map((component) => ({
  ...
  keywords: normalizeKeywords(
    ...
    canonicalComponentNames.has(component.name)
...
const eventItems: DocsSearchItem[] = runtimeEvents.map((event) => {
  const owner = componentDocs.find((component) =>
    component.runtimeEvents?.some((item) => item.name === event.name),
  );
