...
export interface ComponentDoc {
  ...
  category:
    ...
    'Overlays'
  figmaNodeId?: string
  figmaStatus: 'behavior-only' | 'linked'
  glass?: boolean
...
const buildInstallCommand = (
  packages: string[]
) => ([label, command]: typeof packageManagerCommands[number]): InstallCommand => ({
  command: `${command} ${packages.join(' ')}`,
  label
})

const buildInstallCommands = (...packages: string[]): InstallCommand[] => packageManagerCommands.map(
...
export const figmaLibraryUrl =
  'https://www.figma.com/design/luQW2pTQ3jGGxSFPAAsfa9'
...
const figmaNodeIdByComponent: Record<string, string> = {
  ...
  TreeGrid: '51:143',
  Typography: '53:10',
  VisuallyHidden: '513:374',
  VirtualList: '44:24',
  Watermark: '287:19'
}

export const getFigmaComponentUrl = (nodeId?: string) => nodeId ?
...
const dialogTriggerApiRows = (name: string) => [
  apiRow(
    'id', 'string', '-', 'Connects the dialog surface to a matching trigger attribute value.'
  ),
  apiRow(
    `data-ui-${name}-trigger`, 'target id', '-', 'Marks an external trigger and points it at the dialog id.'
  ),
  apiRow(
    `data-ui-${name}-close`, 'boolean attribute', '-', 'Marks a child control that closes the nearest dialog.'
  )
] as const

const keyboardRows = (
  ...rows: [key: string, action: string][]
): KeyboardInteractionRow[] => rows.map(([key, action]) => ({ action, key }))

const disclosureKeyboardInteractions = keyboardRows(
  [
    'ArrowDown, Enter, Space on trigger',
    'Open the panel and move focus to the first focusable item.'
  ], ['Escape in panel', 'Close the panel and return focus to the trigger.'], [
    'ArrowDown, ArrowUp in panel',
    'Move focus through focusable panel items, wrapping at the ends.'
  ], [
    'Home, End in panel',
    'Move focus to the first or last focusable panel item.'
  ]
)
...
const rovingGroupKeyboardInteractions = keyboardRows(
  [
    ...
    'Move roving focus to the next item, wrapping at the end.'
  ], [
    'ArrowLeft, ArrowUp',
    'Move roving focus to the previous item, wrapping at the start.'
...
> = {
  Calendar: keyboardRows(
    ...
    ], ['ArrowRight, ArrowLeft', 'Move day focus by one day.'], ['ArrowDown, ArrowUp', 'Move day focus by one week.'], [
      'Home, End',
      'Move focus to the first or last day in the current week row.'
    ], [
      'PageDown, PageUp',
      'Move focus to the same date in the next or previous month.'
    ]
  ),
  CodeTabs: keyboardRows(
    [
      'ArrowRight',
      'Activate and focus the next code example, wrapping at the end.'
    ], [
      'ArrowLeft',
      'Activate and focus the previous code example, wrapping at the start.'
    ], ['Home, End', 'Activate and focus the first or last code example.']
  ),
  Combobox: keyboardRows(
    [
      'Escape in input or option',
      'Close the listbox. Escape on an option also returns focus to the input.'
    ], [
      'ArrowDown, ArrowUp in input',
      'Open the listbox and move focus to the first or last visible option.'
    ], ['Enter in input', 'Select the first visible option.'], ['Enter, Space on an option', 'Select the focused option.'], [
      'ArrowDown, ArrowUp on an option',
      'Move focus through visible options, wrapping at the ends.'
    ], [
      'Home, End on an option',
      'Move focus to the first or last visible option.'
    ]
  ),
  ContextMenu: keyboardRows(
    [
      ...
      'ArrowDown, ArrowUp in menu',
      'Move focus through focusable menu items, wrapping at the ends.'
    ], [
      'Home, End in menu',
      'Move focus to the first or last focusable menu item.'
    ]
  ),
  DatePicker: keyboardRows(
    ['Enter, Space on trigger', 'Open the calendar popover.'], [
      'ArrowDown on trigger',
      'Open the calendar and move focus to the selected day, today, or first available day.'
    ], [
      'Arrow keys in calendar',
      'Move focus by day or week without opening a browser-native date picker.'
    ], [
      'PageDown, PageUp in calendar',
      'Move to the same date in the next or previous month.'
    ], [
      'Enter, Space on a day',
      'Select the focused date, close the popover, and return focus to the trigger.'
    ], [
      'Escape',
      'Close the calendar without changing the value and return focus to the trigger.'
    ]
  ),
  ...
  ListBox: keyboardRows(
    [
      'Arrow Down / Arrow Up',
      'Moves the active option, skipping disabled options.'
    ], ['Home / End', 'Moves to the first or last enabled option.'], ['Space / Enter', 'Selects or toggles the active option.'], [
      'Type characters',
      'Moves to the first enabled option whose label matches the typed prefix.'
    ]
  ),
  ...
  Resizable: keyboardRows(
    [
      ...
      'Home, End on a handle',
      'Resize the leading panel to its configured minimum or maximum size.'
    ]
  ),
  Select: keyboardRows(
    [
      ...
      'Escape on trigger or option',
      'Close the listbox. Escape on an option also returns focus to the trigger.'
    ], [
      'ArrowDown, ArrowUp on trigger',
      'Open the listbox and focus the next or previous enabled option.'
    ], [
      'Home, End on trigger',
      'Open the listbox and focus the first or last enabled option.'
    ], [
      'Enter, Space on trigger',
      'Open the listbox and focus the selected option, or the first enabled option.'
    ], [
      ...
      'ArrowDown, ArrowUp on an option',
      'Move focus through enabled options, wrapping at the ends.'
    ], [
      'Home, End on an option',
      'Move focus to the first or last enabled option.'
    ]
  ),
  SpeedDial: keyboardRows(
    ['Enter, Space on trigger', 'Open or close the action menu.'], [
      'Arrow keys on trigger',
      'Open the menu and move focus to its first or last action.'
    ], [
      'Arrow keys in menu',
      'Move focus through actions, wrapping at either end.'
    ], ['Home, End in menu', 'Move focus to the first or last action.'], ['Escape', 'Close the action menu and return focus to the trigger.']
  ),
  Tabs: keyboardRows(
    ['ArrowRight', 'Activate and focus the next tab, wrapping at the end.'], [
      'ArrowLeft',
      'Activate and focus the previous tab, wrapping at the start.'
...
export const runtimeEvents: RuntimeEventRow[] = [
  ...
  {
    detail:
      '{ control?: HTMLElement, controls?: HTMLElement[], form: HTMLFormElement }',
    name: 'ui:invalid',
    target: 'Form ([data-ui-form])',
  ...
  {
    detail:
      '{ id: string, title?: string, description?: string, variant?: string, duration?: number }',
...
> = {
  ...
  VirtualList: runtimeEvents.filter(
    event => event.name === 'ui:virtual-list-range'
  )
}
...
const apiReferenceByComponent = {
  ...
  Agenda: [
    apiRow(
      'children', 'Astro slot | ReactNode | child nodes', 'required', 'Provides the chronological headings and semantic event list rendered inside the agenda surface.'
    )
  ],
  Alert: [
    apiRow(
  ...
  Autocomplete: [
    apiRow(
      'list', 'string', 'required', 'Connects the search input to a datalist id.'
    ),
    apiRow(
      'type', 'HTML input type', '"search"', 'Sets the native input type.'
    )
  ],
  Avatar: [
  ...
  BackToTop: [
    ...
    apiRow(
      'data-ui-back-to-top', 'boolean attribute', 'generated', 'Marks the button for scroll-to-top behavior from the UIPrimitives runtime.'
    )
  ],
  Badge: [
    apiRow(
      'variant', '"default" | "secondary" | "outline" | "destructive" | "success" | "warning"', '"default"', 'Controls the badge tone.'
    )
  ],
  BarChart: [
    apiRow(
  ...
  Breadcrumb: [
    apiRow(
      'children', '<ol><li>...</li></ol>', 'required', 'Use an ordered list inside the nav and set aria-current="page" on the final item.'
    )
  ],
  Button: [
    apiRow(
  ...
  ButtonGroup: [
    apiRow(
      'children', 'button controls', 'required', 'Groups related actions while preserving each child button’s native semantics.'
    )
  ],
  ButtonLink: [
    apiRow('href', 'string', '-', 'Sets the link destination.'),
    apiRow(
      'variant', '"primary" | "secondary" | "ghost" | "inline" | "unstyled"', '"primary"', 'Controls the link emphasis or removes presentation for migration use.'
    ),
    apiRow(
  ...
  CardContent: [
    apiRow(
      'as', '"div" | "section"', '"div"', 'Changes the content wrapper without imposing section semantics by default.'
    ),
    apiRow(
      'data-slot', '"card-content"', 'generated', 'Provides the stable public styling hook for the card body.'
    )
  ],
  CardDescription: [
    apiRow(
      'as', '"div" | "p"', '"div"', 'Uses a paragraph only when the supplied content is phrasing content.'
    ),
    apiRow(
      'data-slot', '"card-description"', 'generated', 'Provides the stable public styling hook for supporting copy.'
    )
  ],
  CardFooter: [
    apiRow(
      'as', '"div" | "footer"', '"div"', 'Uses a footer landmark only when the card content has matching sectioning semantics.'
    ),
    apiRow(
      'data-slot', '"card-footer"', 'generated', 'Provides the stable public styling hook for card actions.'
    )
  ],
  CardHeader: [
    apiRow(
      'as', '"div" | "header"', '"div"', 'Uses a header only when the card content has matching sectioning semantics.'
    ),
    apiRow(
      'data-slot', '"card-header"', 'generated', 'Provides the stable public styling hook for introductory content.'
    )
  ],
  CardTitle: [
    apiRow(
      'as', '"div" | "h2" | "h3" | "h4"', '"div"', 'Lets the consumer choose the heading level that matches the page hierarchy.'
    ),
    apiRow(
      'data-slot', '"card-title"', 'generated', 'Provides the stable public styling hook for the card title.'
    )
  ],
  Calendar: [
    apiRow(
  ...
  Callout: [
    apiRow(
      'children', 'phrasing or flow content', 'required', 'Provides the emphasized explanation, caveat, or instruction.'
    )
  ],
  Carousel: [
    apiRow(
      'data-ui-carousel-viewport', 'boolean attribute', '-', 'Marks the scrollable slide viewport.'
    ),
    apiRow(
      'data-ui-carousel-prev, data-ui-carousel-next', 'boolean attribute', '-', 'Marks child controls that move the viewport backward or forward.'
    )
  ],
  Chart: [
    apiRow(
  ...
  Checkbox: [
    apiRow(
      'checked, defaultChecked', 'boolean', '-', 'Uses the native checkbox checked state.'
    ),
  ...
  Collapsible: [
    ...
    apiRow(
      'children', 'summary and flow content', 'required', 'Requires a summary trigger followed by the content controlled by native details behavior.'
    )
  ],
  CoverImage: [
    apiRow(
  ...
  DateRangePicker: [
    apiRow(
      'children', 'two DatePicker controls', 'required', 'Composes the start and end fields from the custom Calendar-backed DatePicker component.'
    ),
    apiRow(
      'onRangeChange', '(range) => void', '-', 'React only: receives the synchronized start and end ISO date values.'
    ),
    apiRow(
      'data-range-state', '"empty" | "selecting-end" | "complete"', 'runtime-managed', 'Exposes the current range progress for styling and product feedback.'
    ),
    apiRow(
      'data-ui-date-range-picker', 'boolean attribute', '-', 'Marks the container so min/max constraints stay synchronized between its DatePicker children.'
    )
  ],
  DatePicker: [
    apiRow(
  ...
  FloatingBadge: [
    apiRow(
      'children', 'number | short text', 'required', 'Provides the compact overlaid notification value.'
    )
  ],
  FormattedDate: [
    apiRow(
      'datetime', 'valid HTML datetime string', 'required', 'Sets the machine-readable date or time value on the rendered time element.'
    ),
    apiRow(
  ...
  Icon: [
    ...
    apiRow(
      'label', 'string', '-', 'Adds an accessible name and exposes the icon as an image.'
    ),
    apiRow(
      'decorative', 'boolean', 'true when unlabelled', 'Keeps decorative icons hidden from assistive technology.'
    ),
    apiRow(
  ...
  Image: [
    apiRow(
      'src', 'ImageMetadata | string', 'required', 'Uses astro:assets in Astro; React passes the source to the native img or the optimizer supplied through as.'
    ),
    apiRow(
    ...
    apiRow(
      'width, height', 'number | string', 'source metadata or required', 'Set intrinsic dimensions for public and remote sources. Astro and Next.js can derive them from supported static imports.'
    ),
    apiRow(
      'widths, sizes', 'number[]; string', 'layout-dependent', 'Astro responsive controls. Next.js also uses sizes to generate and select an efficient width-based srcset.'
    ),
    apiRow(
      'quality, format', 'framework-supported values', 'framework default', 'Passes image encoding choices to astro:assets or to the React optimizer supplied through as.'
    ),
    apiRow(
      'priority', 'boolean', 'false', 'Astro only: marks the likely LCP image for immediate loading, synchronous decoding, and high fetch priority.'
    ),
    apiRow(
      'preload, fill, placeholder', 'NextImage props', 'Next.js defaults', 'Next.js only through as={NextImage}: keeps preloading, fill layout, and placeholder generation under next/image control.'
    ),
    apiRow(
  ...
  InputGroup: [
    apiRow(
      'children', 'input plus prefixes, suffixes, or controls', 'required', 'Composes attached controls while preserving the input’s native label and form behavior.'
    )
  ],
  InputOTP: [
    apiRow(
  ...
  Marker: [
    apiRow(
      'variant', '"default" | "success" | "warning" | "danger"', '"default"', 'Controls the marker tone.'
    )
  ],
  Item: [
    apiRow(
  ...
  Kbd: [
    apiRow(
      'children', 'shortcut text', 'required', 'Provides the key name or shortcut sequence rendered by the semantic kbd element.'
    )
  ],
  Label: [
    apiRow(
      'for', 'element id', '-', 'Associates the label with a form control in Astro and HTML; use htmlFor in React.'
    )
  ],
  LanguageToggle: [
    apiRow(
  ...
  Link: [
    ...
    apiRow(
      'newTab', 'boolean', 'false', 'Opens the destination in a new tab and safely adds noopener and noreferrer to rel.'
    ),
    apiRow(
      'variant', '"default" | "inherit"', '"default"', 'Lets migration surfaces inherit surrounding link color and font weight.'
    ),
    apiRow(
  ...
  Note: [
    apiRow(
      'label', 'string', '-', 'Adds an emphasized label above the note content.'
    ),
    apiRow(
      'borderPosition', '"left" | "right" | "none"', '"left"', 'Positions the accent border, or removes it entirely.'
    )
  ],
  NativeSelect: [
    apiRow(
  ...
  Pagination: [
    apiRow(
      'aria-label', 'string', '"Pagination"', 'Sets the accessible name of the pagination navigation landmark.'
    ),
    apiRow(
      'children', 'links and current-page indicators', 'required', 'Provides native links; use aria-current="page" for the active page and rel for previous and next links.'
    )
  ],
  PhoneInput: [
    apiRow(
  ...
  Pill: [
    apiRow(
      'count', 'number | string', '-', 'Appends a muted count affix after the pill label.'
    ),
    apiRow(
      'href', 'string', '-', 'Renders the pill as a focusable anchor when it represents a destination.'
    ),
    apiRow(
  ...
  Prose: [
    apiRow(
      'children', 'rendered Markdown, CMS, or long-form HTML', 'required', 'Applies readable measure and typographic rhythm without changing the supplied semantics.'
    )
  ],
  RadioGroup: [
    apiRow(
      'children', 'labelled radio inputs', 'required', 'Provides the mutually exclusive native controls inside the fieldset.'
    ),
    apiRow(
  ...
  Schedule: [
    apiRow(
      'data-ui-schedule-event', 'boolean attribute', '-', 'Marks a draggable event and exposes its id to the schedule change event.'
    ),
    apiRow(
  ...
  ScrollArea: [
    apiRow(
      'children', 'scrollable flow content', 'required', 'Provides the content inside the styled overflow container.'
    )
  ],
  ScrollProgress: [
    apiRow(
      'position', '"top" | "bottom"', '"top"', 'Pins the progress indicator to a viewport edge.'
    ),
    apiRow(
      'aria-label', 'string', '"Reading progress"', 'Names the live document-progress indicator for assistive technology.'
    )
  ],
  Select: [
    apiRow(
  ...
  Separator: [
    apiRow(
      'orientation', '"horizontal" | "vertical"', '"horizontal"', 'Sets the separator direction.'
    )
  ],
  SearchField: [
    apiRow(
  ...
  Skeleton: [
    apiRow(
      'aria-label, aria-labelledby', 'string', '-', 'Makes the placeholder accessible and changes the default role to status.'
    ),
    apiRow(
      'aria-hidden', 'ARIA boolean string', 'true unless labelled', 'Overrides whether the loading placeholder is exposed to assistive technology.'
    ),
    apiRow(
  ...
  Spinner: [
    apiRow(
      'aria-label, aria-labelledby', 'string', '-', 'Makes the spinner accessible and changes the default role to status.'
    ),
    apiRow(
      'aria-hidden', 'ARIA boolean string', 'true unless labelled', 'Overrides whether the loading indicator is exposed to assistive technology.'
    ),
    apiRow(
      'role', 'ARIA role', '"status" when labelled', 'Overrides the generated status role.'
    )
  ...
  Switch: [
    ...
    apiRow(
      'role', 'ARIA role', '"switch"', 'Sets the switch role on the checkbox input.'
    ),
    apiRow('type', '"checkbox"', '"checkbox"', 'Fixed by the component.')
  ],
  Table: [
  ...
  TagGroup: [
    apiRow(
      'data-ui-tag', 'boolean attribute', '-', 'Marks each tag item for group behavior.'
    ),
    apiRow(
      'data-ui-tag-remove', 'boolean attribute', '-', 'Marks a labelled child control that removes its closest tag and emits ui:tag-remove.'
    )
  ],
  ...
  ThemeToggle: [
    apiRow(
      'storageKey', 'string', '"lumen-theme"', 'Sets the local-storage key used to persist the selected theme.'
    ),
    apiRow(
      'lightTheme, darkTheme', 'string', '"light", "dark"', 'Sets the data-theme values toggled on the document element.'
    ),
    apiRow(
      'controlled', 'boolean', 'false', 'Disables automatic theme changes when the host application owns theme state.'
    )
  ],
  Toast: [
    apiRow(
  ...
  Tooltip: [
    disclosureTriggerApiRow,
    apiRow(
      '[role="tooltip"]', 'child panel', 'required', 'Provides the non-interactive explanation linked to the trigger with aria-describedby.'
    )
  ],
  TimeField: [
  ...
  Toggle: [
    apiRow(
      'controlled', 'boolean', 'false', 'Disables automatic Astro state changes while retaining the cancelable ui:toggle-intent event.'
    ),
  ...
  ToggleGroup: [
    apiRow(
      'data-ui-toggle-group', 'boolean attribute', '-', 'Marks the container for roving focus management by the UIPrimitives runtime.'
    )
  ],
  Tree: [
    apiRow(
      'role', 'ARIA role', '"tree"', 'Generated tree role for the root element.'
    ),
    apiRow(
      'aria-expanded, aria-level', 'ARIA attributes', '-', 'Use on child items to indicate nested tree structure.'
    )
  ],
  TreeGrid: [
    apiRow(
      'role', 'ARIA role', '"treegrid"', 'Generated treegrid role for the root element.'
    ),
    apiRow(
      'role="row", role="gridcell"', 'ARIA roles', '-', 'Use on children to build out the hierarchical grid rows and cells.'
    )
  ],
  Typography: [
    apiRow(
      'children', 'semantic HTML', 'required', 'Styles descendant headings, paragraphs, lists, blockquotes, and links without changing their HTML semantics.'
    )
  ],
  VirtualList: [
    apiRow(
  ...
  Particles: [
    apiRow(
      'density', '"low" | "medium" | "high"', '"medium"', 'Controls the density of the particles.'
    )
  ],
  AnimatedNumber: [
    apiRow(
      'value', 'number', 'required', 'Sets the final value announced to assistive technology and displayed after the animation.'
    ),
    apiRow('from', 'number', '0', 'Sets the visual starting value.'),
    apiRow(
      'decimals', 'number', '0', 'Sets the number of displayed decimal places.'
    ),
    apiRow(
      'locale', 'string', 'browser locale', 'Formats the value with Intl.NumberFormat.'
    ),
    apiRow(
      'prefix, suffix', 'string', '""', 'Adds stable text around the formatted number.'
    ),
    apiRow(
  ...
  StatDescription: [
    apiRow(
      'as', '"div" | "p"', '"div"', 'Uses a paragraph only when the supporting content is phrasing content.'
    ),
    apiRow(
      'data-slot', '"stat-description"', 'generated', 'Provides the stable public styling hook for metric context.'
    )
  ],
  StatIcon: [
    apiRow(
      'children', 'Icon or custom artwork', 'required', 'Adds a decorative or labelled visual associated with the metric.'
    ),
    apiRow(
      'data-slot', '"stat-icon"', 'generated', 'Provides the stable public styling hook for the icon container.'
    )
  ],
  StatLabel: [
    apiRow(
      'as', '"div" | "span"', '"div"', 'Changes the label wrapper without adding heading semantics.'
    ),
    apiRow(
      'data-slot', '"stat-label"', 'generated', 'Provides the stable public styling hook for the metric label.'
    )
  ],
  StatTrend: [
    apiRow(
      'tone', '"neutral" | "success" | "warning" | "danger"', '"neutral"', 'Applies a semantic trend tone; visible text or an accessible name must communicate its meaning.'
    ),
    apiRow(
      'data-slot', '"stat-trend"', 'generated', 'Provides the stable public styling hook for a delta or comparison.'
    )
  ],
  StatValue: [
    apiRow(
      'as', '"div" | "output" | "span"', '"div"', 'Uses output only when the value is the result of a user action or calculation.'
    ),
    apiRow(
      'data-slot', '"stat-value"', 'generated', 'Provides the stable public styling hook for the prominent value.'
    )
  ],
  Meter: [
    apiRow('value', 'number', 'required', 'Sets the current scalar value.'),
    apiRow(
      'min, max', 'number', '0, 100', 'Sets the inclusive measurement range.'
    ),
    apiRow(
      'low, high, optimum', 'number', '-', 'Defines qualitative ranges for the native meter element.'
    )
  ],
  Rating: [
    ...
    apiRow(
      'readonly', 'boolean', 'false', 'Presents the rating without interactive score controls.'
    )
  ],
  Timeline: [
    apiRow(
      'children', 'TimelineItem elements', 'required', 'Provides chronologically ordered milestones or activity events.'
    )
  ],
  GradientDivider: [
    apiRow(
      'aria-hidden', 'ARIA boolean string', '"true"', 'Keeps the decorative gradient out of the accessibility tree unless explicitly overridden.'
    )
  ],
  Stepper: [
    apiRow(
      'steps', 'Array<string | { label, description? }>', '[]', 'Defines the ordered steps.'
    ),
    apiRow('currentStep', 'number', '0', 'Sets the zero-based active step.'),
    apiRow(
      'orientation', '"horizontal" | "vertical"', '"horizontal"', 'Controls the step layout direction.'
    )
  ],
  FileUpload: [
    apiRow(
    ...
    apiRow(
      'hint', 'string', '-', 'Adds supporting file type or size guidance.'
    ),
    apiRow(
      'accept, multiple, name', 'native file input attributes', '-', 'Controls accepted files and form submission.'
    )
  ],
  Tour: [
    apiRow(
      'steps', 'Array<{ target, content, title? }>', '[]', 'Defines the ordered anchored guidance steps.'
    ),
    apiRow(
      'nextLabel, closeLabel', 'string', '"Next", "Close"', 'Sets the tour navigation action labels.'
    )
  ],
  Anchor: [
    apiRow(
  ...
  Segmented: [
    apiRow(
      'options', 'Array<string | { label, value, disabled? }>', '[]', 'Defines the mutually exclusive visible options.'
    ),
    apiRow(
      'value, defaultValue', 'string', '-', 'Controls or initializes the selected value.'
    ),
    apiRow('name', 'string', '-', 'Sets the submitted form field name.'),
    apiRow(
      'size', '"default" | "sm" | "lg"', '"default"', 'Controls the segment height and spacing.'
    )
  ],
  Toolbar: [
    apiRow(
      'orientation', '"horizontal" | "vertical"', '"horizontal"', 'Sets the toolbar direction and keyboard navigation axis.'
    ),
    apiRow(
  ...
  Descriptions: [
    ...
    apiRow(
      'columns', 'number', '1', 'Sets the preferred number of aligned columns.'
    )
  ],
  Popconfirm: [
    apiRow(
      'title', 'string', '"Are you sure?"', 'Sets the concise confirmation question.'
    ),
    apiRow(
      'confirmLabel, cancelLabel', 'string', '"Confirm", "Cancel"', 'Sets the action labels.'
    )
  ],
  Transfer: [
    apiRow(
      'items, targetItems', 'TransferItem[]', '[]', 'Defines the available and initially chosen collections.'
    ),
    apiRow(
      'sourceTitle, targetTitle', 'string', '"Available", "Selected"', 'Labels the two collections.'
    ),
    apiRow('name', 'string', '-', 'Sets the submitted values field name.')
  ],
  Cascader: [
    apiRow(
      'options', 'CascaderOption[]', '[]', 'Defines the hierarchical option branches.'
    ),
    apiRow('name', 'string', '-', 'Sets the submitted field name.'),
    apiRow(
      'placeholder', 'string', '"Select"', 'Sets the trigger text before a path is selected.'
    )
  ],
  TreeSelect: [
    ...
    apiRow('name', 'string', '-', 'Sets the submitted field name.'),
    apiRow(
      'placeholder', 'string', '"Select"', 'Sets the trigger text before a node is selected.'
    )
  ],
  Mentions: [
    apiRow(
      'options', 'Array<string | { label, value }>', '[]', 'Defines the mention suggestions.'
    ),
    apiRow(
      'trigger', 'string', '"@"', 'Sets the character that opens suggestions.'
    ),
    apiRow(
  ...
  QRCode: [
    apiRow(
      'value', 'string', 'required unless src is provided', 'Generates a QR code from the supplied value.'
    ),
    apiRow(
      'src', 'string', '-', 'Uses an existing QR image instead of generating one.'
    ),
    apiRow('size', 'number', '160', 'Sets the rendered square dimensions.')
  ],
  Watermark: [
    apiRow(
      'content, text', 'string', '"Lumen"', 'Sets the repeated watermark text.'
    ),
    apiRow('gap', 'number', '96', 'Sets spacing between watermark marks.'),
    apiRow(
      'rotate', 'number', '-22', 'Sets the watermark rotation in degrees.'
    )
  ],
  Affix: [
    apiRow(
      'position', '"top" | "bottom"', '"top"', 'Selects the viewport edge used for sticky positioning.'
    ),
    apiRow(
      'offset, offsetTop, offsetBottom', 'number', '0', 'Sets the sticky inset in pixels; edge-specific values override offset.'
    )
  ],
  CheckboxGroup: [
    apiRow(
      'legend', 'string', '-', 'Adds the visible fieldset legend for the related checkbox choices.'
    ),
    apiRow(
      'orientation', '"horizontal" | "vertical"', '"vertical"', 'Controls the visual arrangement without changing fieldset semantics.'
    ),
    apiRow(
      'invalid', 'boolean', 'false', 'Marks the group invalid for validation feedback.'
    )
  ],
  Container: [
    apiRow(
  ...
  ErrorSummary: [
    apiRow(
      'errors', 'LumenFormErrorInput', '-', 'Normalizes form and field errors into a linked summary.'
    ),
    apiRow(
      'heading', 'string', '"There is a problem"', 'Sets the summary heading.'
    ),
    apiRow(
      'id', 'string', '"ui-error-summary"', 'Sets the focus target and heading relationship.'
    )
  ],
  FieldError: [
    apiRow(
      'message', 'string', '-', 'Shows the error text and reveals the live field error region.'
    ),
    apiRow(
      'children', 'phrasing content', '-', 'Overrides the message prop with authored content.'
    )
  ],
  Form: [
    apiRow(
      'enhance', 'boolean', 'true', 'Enables Lumen constraint-validation enhancement.'
    ),
    apiRow(
      'status', '"idle" | "submitting" | "success" | "error"', '"idle"', 'Exposes submission state and aria-busy.'
    )
  ],
  Grid: [
    apiRow(
      'as', '"div" | "ol" | "section" | "ul"', '"div"', 'Changes the semantic root.'
    ),
    apiRow(
  ...
  ListBox: [
    apiRow(
      'options', 'Array<string | { label, value, disabled?, section? }>', '[]', 'Defines options and optional accessible sections.'
    ),
    apiRow(
      'value', 'string | string[]', '-', 'Controls the selected value or values.'
    ),
    apiRow(
      'multiple', 'boolean', 'false', 'Allows more than one selected value while preserving native form submission.'
    ),
    apiRow(
      'loading', 'boolean', 'false', 'Marks the collection busy and disables selection while options load.'
    ),
    apiRow(
      'emptyMessage', 'string', '"No options available"', 'Labels the empty collection state.'
    ),
    apiRow(
      'selectionFollowsFocus', 'boolean', 'false', 'Selects each enabled option as keyboard focus moves instead of waiting for Space or Enter.'
    )
  ],
  PasswordField: [
    apiRow('label', 'string', '"Password"', 'Labels the password input.'),
    apiRow(
      'hint, error', 'string', '-', 'Adds described hint or error content.'
    ),
    apiRow(
      'showLabel, hideLabel', 'string', '"Show password", "Hide password"', 'Names the visibility toggle states.'
    )
  ],
  Stack: [
    apiRow(
      'as', '"article" | "div" | "fieldset" | "nav" | "section"', '"div"', 'Changes the semantic root.'
    ),
    apiRow(
  ...
  VisuallyHidden: [
    apiRow(
      'focusable', 'boolean', 'false', 'Reveals the content on focus for keyboard-accessible skip or helper controls.'
    ),
    apiRow(
      'children', 'phrasing content', 'required', 'Provides content for assistive technology.'
    )
  ]
...
const componentGuidanceByName: Partial<Record<string, ComponentGuidance>> = {
  ...
  Alert: {
    ...
    distinction:
      'Use Toast for temporary feedback, Callout for editorial emphasis, or Note for supporting context.'
  },
  AlertDialog: {
    when: 'Use before an irreversible or high-impact action that requires an explicit decision.',
    distinction:
      'Use Dialog for ordinary focused tasks and Popconfirm for a compact confirmation beside its trigger.'
  },
  Autocomplete: {
    when: 'Use when people may type any value but benefit from browser-native suggestions.',
    distinction:
      'Use Combobox when the value must come from a managed option list with richer keyboard behavior.'
  },
  Badge: {
  ...
  Bubble: {
    ...
    distinction:
      'Use Message to compose the full row, including avatar, author, timestamp, and a Bubble.'
  },
  ButtonLink: {
    when: 'Use for navigation that needs button-level visual emphasis while preserving link behavior.',
    distinction:
      'Use Button for an action that changes state and Link for ordinary inline navigation.'
  },
  Callout: {
    when: 'Use to pull an important explanation, caveat, or instruction out of surrounding prose.',
    distinction:
      'Use Alert for a system status or outcome and Note for quieter supplementary information.'
  },
  Collapsible: {
    when: 'Use for one optional region such as advanced settings, details, or filters.',
    distinction:
      'Use Accordion when several related disclosures need shared structure.'
  },
  Combobox: {
  ...
  DataTable: {
    ...
    distinction:
      'Use Table for static semantic rows and columns, and TreeGrid for expandable hierarchical rows.'
  },
  Dialog: {
  ...
  Marker: {
    ...
    distinction:
      'Use Badge for a filled metadata label and Pill for a label paired with a count.'
  },
  Meter: {
  ...
  NativeSelect: {
    ...
    distinction:
      'Use Select for Lumen’s enhanced custom control or Combobox when people need to search the options.'
  },
  Note: {
    when: 'Use for supplementary information that should stay attached to nearby content.',
    distinction:
      'Use Callout for stronger editorial emphasis and Alert for status that requires attention.'
  },
  PieChart: {
  ...
  Popconfirm: {
    ...
    distinction:
      'Use AlertDialog when the consequence is serious enough to require a modal interruption.'
  },
  Popover: {
    when: 'Use for contextual floating content that may contain controls, actions, or a small form.',
    distinction:
      'Use Tooltip for one short explanation and HoverCard for a read-only preview.'
  },
  Progress: {
  ...
  Select: {
    ...
    distinction:
      'Use NativeSelect for platform-native behavior and Combobox when the option list needs text search.'
  },
  Sheet: {
  ...
  Sparkline: {
    ...
    distinction:
      'Use LineChart when axes, multiple series, thresholds, exact values, or a data table are important.'
  },
  Stat: {
  ...
  Toggle: {
    ...
    distinction:
      'Use Switch for an immediate setting and Checkbox for a form value or agreement.'
  },
  ToggleGroup: {
  ...
  Tooltip: {
    ...
    distinction:
      'Use HoverCard for a richer preview and Popover for interactive floating content.'
  },
  Typography: {
  ...
  TreeGrid: {
    ...
    distinction:
      'Use Tree for hierarchy without columns and DataTable for flat interactive records.'
  }
}
...
export const componentDocs: ComponentDoc[] = (
  [
    ...
    [
      ...
      '<Card><CardDescription>Updated five minutes ago.</CardDescription></Card>'
    ],
    [
      'CardFooter',
      'Layout',
      'Groups actions at the end of a Card.',
      '<Card><CardFooter><Button>Continue</Button></CardFooter></Card>'
    ],
    [
      'CardHeader',
      'Layout',
      'Groups introductory content at the start of a Card.',
      '<Card><CardHeader><h2>Workspace</h2></CardHeader></Card>'
    ],
    [
      'CardTitle',
      'Layout',
      'Provides a semantic Card title with a stable styling hook.',
      '<Card><CardTitle as="h2">Workspace</CardTitle></Card>'
    ],
    [
      'Carousel',
      'Layout',
    ...
    [
      ...
      '<CheckboxGroup legend="Notifications"><label><Checkbox name="alerts" /> Product alerts</label></CheckboxGroup>'
    ],
    [
      'Collapsible',
      'Layout',
    ...
    [
      ...
      '<ErrorSummary errors={{ fields: [{ controlId: "email", name: "email", message: "Enter an email address." }], form: [] }} />'
    ],
    [
      'Eyebrow',
      'Data display',
    ...
    [
      ...
      '<FieldError id="email-error" message="Enter a valid email address." />'
    ],
    [
      'FloatingBadge',
      'Data display',
    ...
    [
      ...
      '<Form status="idle"><Button type="submit">Save changes</Button></Form>'
    ],
    [
      'FormattedDate',
      'Data display',
    ...
    [
      ...
      '<p>Open the command menu with <Kbd>Cmd K</Kbd>.</p>'
    ],
    [
      'Label',
      'Forms',
    ...
    [
      ...
      '<ListBox label="Team members" multiple options={["Amina", "Theo", "Luca"]} value={["Amina"]} />'
    ],
    [
      'Marker',
      'Data display',
    ...
    [
      ...
      '<PasswordField name="password" hint="Use at least 12 characters." />'
    ],
    [
      'PhoneInput',
      'Forms',
    ...
    [
      ...
      '<Stat><StatDescription>Across all workspaces</StatDescription></Stat>'
    ],
    [
      'StatIcon',
      'Data display',
      'Provides an icon region for a composed statistic.',
      '<Stat><StatIcon><Icon name="users" decorative /></StatIcon></Stat>'
    ],
    [
      'StatLabel',
      'Data display',
      'Labels a composed statistic.',
      '<Stat><StatLabel>Active workspaces</StatLabel></Stat>'
    ],
    [
      'StatTrend',
      'Data display',
      'Communicates a metric delta with a semantic tone.',
      '<Stat><StatTrend tone="success">Up 12%</StatTrend></Stat>'
    ],
    [
      'StatValue',
      'Data display',
      'Displays the prominent value in a composed statistic.',
      '<Stat><StatValue>1,234</StatValue></Stat>'
    ],
    [
      'Stack',
      'Layout',
      'Arranges children with tokenized direction, spacing, and alignment.',
      '<Stack direction="horizontal" gap="sm"><Button>Save</Button><Button variant="secondary">Cancel</Button></Stack>'
    ],
    [
      'Meter',
      'Data display',
  ...
  ...(figmaNodeIdByComponent[name] ?
    { figmaNodeId: figmaNodeIdByComponent[name] } :
    {}),
  figmaStatus: figmaNodeIdByComponent[name] ? 'linked' : 'behavior-only',
  glass: glassComponentNameSet.has(name),
  ...(componentGuidanceByName[name] ?
    { guidance: componentGuidanceByName[name] } :
    {}),
  ...
  name,
  category,
  ...(runtimeEventsByComponent[name] ?
    { runtimeEvents: [...runtimeEventsByComponent[name]] } :
    {}),
  summary,
  example
}))
...
export function VerificationCode() {
  ...
  },
  {
    ...
    controller: [
      ...
      hookApiRow(
        'executeCommand', '(command: string, root?, value?) => boolean', '-', 'Executes a rich text command with an optional value.'
      ),
      hookApiRow(
...
export const elementsApiReference: ElementsApiDoc[] = [
  {
    ...
    rows: [
      elementsRow(
        'registry', 'CustomElementRegistry (optional)', 'Custom element registry to register against. Defaults to the global customElements.'
      )
    ],
    code: `<script type="module">
  import { defineLumenElements } from '@santi020k/lumen-elements/define'
...
<lumen-card>
  ...
  },
  {
    ...
    rows: [
      elementsRow(
        'root', 'ParentNode (optional)', 'Root element to search for [data-ui-form] forms. Defaults to document.'
      ),
      elementsRow(
        'data-error-required', 'attribute', 'Custom message shown when a required field is empty.'
      ),
      elementsRow(
        'data-error-type', 'attribute', 'Custom message shown when the value does not match the input type (e.g. email).'
      ),
      elementsRow(
        'data-error-pattern', 'attribute', 'Custom message shown when the value does not match the pattern attribute.'
      ),
      elementsRow(
        'data-error-min / data-error-max', 'attribute', 'Custom message for range violations.'
      )
    ],
    code: `<script type="module">
...
<form data-ui-form>
  ...
  },
  {
    ...
    rows: [
      elementsRow(
        'root', 'ParentNode (optional)', 'Root element to search for calendars. Defaults to document.'
      ),
      elementsRow('month', 'YYYY-MM', 'Visible month for generated calendars.'),
      elementsRow(
        'value', 'YYYY-MM-DD', 'Initial selected date copied to the hidden input.'
      ),
      elementsRow(
        'min / max', 'YYYY-MM-DD', 'Inclusive selectable date bounds.'
      ),
      elementsRow(
        'name', 'attribute', 'Forwarded to the generated hidden input for form submission.'
      )
    ],
    code: `<script type="module">
...
></lumen-calendar>`
  },
  {
    ...
    rows: [
      elementsRow(
        'root', 'ParentNode (optional)', 'Root element to search for date range pickers. Defaults to document.'
      ),
      elementsRow(
        'input[type="date"]', 'native date inputs', 'The first date input is treated as the start date and the last date input is treated as the end date.'
      )
    ],
    code: `<script type="module">
...
<section data-ui-date-range-picker class="ui-date-range-picker">
  ...
  },
  {
    ...
    rows: [
      elementsRow(
        'root', 'ParentNode (optional)', 'Root element to search for OTP fields. Defaults to document.'
      ),
      elementsRow(
        'length', 'attribute', 'Number of OTP segments to create. Defaults to 6.'
      ),
      elementsRow(
        'value', 'attribute', 'Initial value copied to the generated native input.'
      ),
      elementsRow(
        'name', 'attribute', 'Forwarded to the generated native input for form submission.'
      )
    ],
    code: `<script type="module">
...
></lumen-input-otp>`
  },
  {
    ...
    rows: [
      elementsRow(
        'root', 'ParentNode (optional)', 'Root element to search for resizable groups. Defaults to document.'
      ),
      elementsRow(
        'data-ui-resizable-default-sizes', 'comma-separated percentages', 'Initial pane sizes normalized to 100%.'
      ),
      elementsRow(
        'data-ui-resizable-min-size', 'number or comma list', 'Minimum pane size percentage. Defaults to 12.'
      ),
      elementsRow(
        'data-ui-resizable-max-size', 'number or comma list', 'Maximum pane size percentage. Defaults to 88.'
      ),
      elementsRow(
        'data-orientation', '"horizontal" | "vertical"', 'Resize axis. Defaults to horizontal.'
      )
    ],
    code: `<script type="module">
...
<lumen-resizable data-ui-resizable-default-sizes="30,70">
  ...
  },
  {
    ...
    rows: [
      elementsRow(
        'root', 'ParentNode (optional)', 'Root element to search for editors. Defaults to document.'
      ),
      elementsRow(
        'data-ui-command', 'attribute', 'Set on toolbar buttons to specify the command name (bold, italic, insertUnorderedList, etc.).'
      )
    ],
    code: `<script type="module">
...
<section data-ui-rich-text-editor class="ui-rich-text-editor">
  ...
  },
  {
    ...
    rows: [
      elementsRow(
        'create(detail)', '(ToastDetail) => string', 'Create a toast with title, description, variant, duration, placement, and optional action. Returns the toast id.'
      ),
      elementsRow(
        'dismiss(id?)', '(string?) => void', 'Dismiss a toast by id, or dismiss the most recent toast.'
      ),
      elementsRow(
        'update(id, detail)', '(string, ToastDetail) => void', 'Update an existing toast with new content.'
      )
    ],
    code: `<script type="module">
...
<lumen-button id="notify">Show toast</lumen-button>`
  },
  {
    ...
    rows: [
      elementsRow(
        'ui:toast', 'CustomEvent<ToastDetail>', 'Dispatched on document when a toast is created. Listen on document to handle toasts globally.'
      ),
      elementsRow(
        'ui:validate', 'CustomEvent<{ control, form, value }>', 'Dispatched during form validation for each control.'
      ),
      elementsRow(
        'ui:valid', 'CustomEvent<{ form, controls? }>', 'Dispatched when a form passes validation.'
      ),
      elementsRow(
        'ui:invalid', 'CustomEvent<{ form, control?, controls? }>', 'Dispatched when a form fails validation.'
      ),
      elementsRow(
        'ui:editor-command', 'CustomEvent<{ command }>', 'Dispatched when a rich text editor command is executed.'
      ),
      elementsRow(
        'ui:toast-action', 'CustomEvent<{ event?, value? }>', 'Dispatched when a toast action button is clicked.'
      )
    ],
    code: `<script type="module">
