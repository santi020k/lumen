...
export const getFigmaComponentUrl = (nodeId?: string) => nodeId ? `${figmaLibraryUrl}?node-id=${nodeId.replace(':', '-')}` : figmaLibraryUrl
...
const keyboardInteractionsByComponent: Partial<Record<string, readonly KeyboardInteractionRow[]>> = {
  Calendar: keyboardRows(
    ...
    ], ['ArrowRight, ArrowLeft', 'Move day focus by one day.'], ['ArrowDown, ArrowUp', 'Move day focus by one week.'], ['Home, End', 'Move focus to the first or last day in the current week row.'], ['PageDown, PageUp', 'Move focus to the same date in the next or previous month.']
  ),
  ...
  ContextMenu: keyboardRows(
    ...
    ], ['Escape in menu', 'Close the menu and return focus to the trigger.'], ['ArrowDown, ArrowUp in menu', 'Move focus through focusable menu items, wrapping at the ends.'], ['Home, End in menu', 'Move focus to the first or last focusable menu item.']
  ),
  ...
  Select: keyboardRows(
    ...
    ], ['ArrowDown, ArrowUp on an option', 'Move focus through enabled options, wrapping at the ends.'], ['Home, End on an option', 'Move focus to the first or last enabled option.']
  ),
...
const apiReferenceByComponent = {
  ...
  BarChart: [
    ...
    apiRow('categoryWidth', 'number', '112', 'Reserves 64–240 SVG units for horizontal category labels.'),
    apiRow(
