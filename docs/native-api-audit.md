...
| SwiftUI | Current symbol graphs: 3,063 macOS, 3,040 iOS, 2,879 tvOS, 3,040 visionOS, and 2,899 watchOS symbols | Every classified symbol is Supported on each platform; 0 Experimental, Deprecated, or Unclassified | `pnpm run check:swift-api-baseline` rebuilds every declared platform and compares it with `registry/swift-api-baseline.json`; `pnpm run check:swift-source-compatibility` currently reports 10 unreviewed diagnostics beyond the four approved Lumen 2 enum additions relative to `v1.6.0` | Review the outstanding compatibility diagnostics, then restart the two-iteration soak against the new baseline |
