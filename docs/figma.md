...
Foundations, Glass Components, Icons, component-category, and Utilities pages. The published Figma
file currently uses Inter for interface and documentation text, while Lumen's runtime CSS defaults
`--ui-font` to the canonical Montserrat family stack. Figma text styles do not change through token
export, so treat this as an explicit design-file difference until those styles and specimens are
migrated together. Cascadia Mono remains reserved for the small amount of code-style text.
