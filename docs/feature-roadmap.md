...
- React behavior hooks: `useDialog`, `usePopover`, `useDropdownMenu`, `useTabs`, `useSelect`,
  `useFormValidation`, `useRichTextEditor`, `useToast`, and `useTooltip` expose the main behavior
  contracts for React apps.
...
- Remaining Web Components behavior parity: `packages/elements` now covers DataTable, Dialog,
  Popover, DropdownMenu, Tabs, Select, ThemeBuilder, Toast, Tooltip, forms, RichTextEditor, and
  VirtualList; Schedule and other advanced runtimes still need equivalent client behavior.
