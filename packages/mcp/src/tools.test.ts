...
describe('lumen-mcp data snapshot', () => {
  ...
  test('documents wrapped code and Astro motion runtime requirements', () => {
    ...
    for (const name of ['AnimatedNumber', 'RevealGroup', 'ScrollReveal']) {
      expect(resolveComponent(name, data)?.dependencies).toEqual(expect.arrayContaining(['runtime', 'styles']))
    }
  })
  ...
  test('matches package versions and contains deterministic freshness metadata', () => {
    ...
    const packageJson = JSON.parse(readFsFile(new URL('../package.json', import.meta.url), 'utf8')) as {
      version: string
    }
...
describe('search', () => {
  ...
  test('returns a clear response when nothing matches', () => {
    expect(search({ query: 'definitely-not-in-lumen' }).text).toBe('No matches for "definitely-not-in-lumen".')
  })
})
