...
const appendSearchResult = (
  ...
  const match = scoreCandidate(
    ...
    terms,
    searchableText,
    candidate.kind === "rule" ? "" : candidate.name,
  );
  ...
  const matches = data.components.filter((component) => {
    ...
    if (
      ...
      scoreCandidate(
        query,
        queryTerms,
        componentSearchText(component, args.framework),
        component.name,
      ).score === 0
    )
      return false;
...
const diffManifestSection = (
  ...
  return {
    ...
    removed: baselineNames
      .filter(
        (name) =>
          (Reflect.get(current, name) as string | undefined) === undefined,
      )
      .sort(),
...
export const diffCatalog = (
  ...
  const components = diffManifestSection(
    data.catalogManifest.components,
    args.baseline.components,
  );
  const recipes = diffManifestSection(
    data.catalogManifest.recipes,
    args.baseline.recipes,
  );
...
export const diagnose = (
  ...
  const checks = [
    ...
    },
    {
      ...
      status: componentNames.every((name) =>
        Reflect.get(data.catalogManifest.components, name),
      )
        ? "pass"
        : "fail",
    },
