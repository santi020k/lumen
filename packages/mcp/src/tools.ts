...
): LumenFramework[] => (Object.entries(component.frameworks) as [LumenFramework, boolean][])
...
const frameworkBehaviorSearchText = (
  behavior: LumenFrameworkSnapshot['behavior']
) => [
...
export const listComponents = (
  ...
  const recipeName =
    recipe?.name ?? (args.recipe ? normalize(args.recipe) : undefined)
  ...
  const matches = data.components.filter(component => {
    ...
    if (
      recipeName &&
      !component.recipes.some(
        componentRecipe => normalize(componentRecipe.name) === recipeName
      )
    )
      return false
...
const componentDetail = (
  ...
  const selectedFramework = getFrameworkDetail(
...
const appendBehaviorSections = (
  ...
  if (behavior.options && behavior.options.length > 0) {
    sections.push(
      '', 'Hook options:', ...behavior.options.map(
        option => `- ${option.name}: ${option.type} — ${option.description}`
      )
    )
  }
  ...
  if (behavior.controller && behavior.controller.length > 0) {
    sections.push(
      '', 'Controller values:', ...behavior.controller.map(
        value => `- ${value.name}: ${value.type} — ${value.description}`
      )
    )
  }
}

const appendGuidanceSections = (
  sections: string[],
  component: LumenComponentSnapshot
) => {
  if (component.guidance) {
    ...
    if (component.guidance.distinction)
      sections.push(component.guidance.distinction)
  }
  ...
  if (component.runtimeEvents.length > 0) {
    ...
    sections.push(
      ...component.runtimeEvents.map(
        event => `- ${event.name}: ${event.when}`
      )
    )
  }
}
...
const formatComponentUsage = (
  ...
  const framework = getFrameworkDetail(
    component.frameworkDetails, frameworkName
  )
  ...
  sections.push(
    '', formatProps(framework), '', '## Example', `\`\`\`${framework.language}`, framework.example, '```'
  )
  ...
  if (detail === 'source') {
    sections.push(
      '', `## ${frameworkName} reference source`, `\`\`\`${framework.language}`, framework.source.trimEnd(), '```'
    )
  }
...
const diffManifestSection = (
  ...
  return {
    added: currentNames
      .filter(
        name => (Reflect.get(baseline, name) as string | undefined) === undefined
      )
      .sort(),
    changed: currentNames
      ...
      .sort(),
    removed: baselineNames
      .filter(
        name => (Reflect.get(current, name) as string | undefined) === undefined
      )
      .sort(),
    unchanged: currentNames
      .filter(
        name => Reflect.get(baseline, name) === Reflect.get(current, name)
      )
      .sort()
  }
}
...
export const diffCatalog = (
  ...
  const components = diffManifestSection(
    data.catalogManifest.components, args.baseline.components
  )

  const recipes = diffManifestSection(
  ...
  const checks = [
    {
      ...
      status:
        data.components.length === data.meta.componentCount ? 'pass' : 'fail'
    },
    {
    ...
    },
    {
      ...
      status: componentNames.every(name => Reflect.get(data.catalogManifest.components, name)) ?
        'pass' :
        'fail'
    },
    {
      message:
        'Every available framework contract has an example and import statement.',
...
export const getTokens = (
  ...
  const chartLines = Object.entries(tokens.chart).map(
  ...
  const glassLines = Object.entries(tokens.glass).map(
    ([name, value]) => `- ${name}: ${value}`
  )
