...
describe('@santi020k/lumen-react components', () => {
  ...
  test('applies input size modifiers and typed input classes', () => {
    expect(propsOf(Input({ size: 'sm' }) as ReactElement).className).toBe('ui-input ui-input--sm')
    expect(propsOf(Input({ size: 'lg' }) as ReactElement).className).toBe('ui-input ui-input--lg')
